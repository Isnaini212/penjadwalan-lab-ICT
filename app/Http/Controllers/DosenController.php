    ...
    if ($dayOfWeek === Carbon::SATURDAY) {
        ...
        if ($startIndex !== false) {
            ...
            if ((int)$request->sks > $maxSks) {
                return back()->withInput()->withErrors(['sks' => "Gagal! Untuk jam mulai {$jam_mulai_formatted}, SKS maksimal yang diperbolehkan adalah {$maxSks} agar waktu selesai tidak melewati 20:40."]);
            }
        } else {
            if ((int)$request->sks < 1 || (int)$request->sks > 4) {
                return back()->withInput()->withErrors(['sks' => 'Gagal! SKS harus bernilai antara 1 sampai 4.']);
            }
        }
    }
